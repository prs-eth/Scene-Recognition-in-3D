...
def validate(net,val_loader,device,config): 
    ...
    np.savez('mix_train.npz',x=x,y=y)
...
def train(net, loaders, device, logger, config):
    ...
    for curr_iter in range(restore_iter, config['max_iter']):
        try:
            # if(curr_iter % config['freq_step_lr']==0):
            #     adjust_learning_rate(optimizer)
                
            # train on one batch and optimize
            data_dict = train_loader.next()
            
            # if(data_dict['feats'].size()[0]>config['train_limit_numpoint']):  # fix memory error
            #     indice=np.random.permutation(data_dict['feats'].size()[0])
            #     trun_indice=indice[:config['train_limit_numpoint']]
            #     data_dict['feats']=data_dict['feats'][trun_indice]
            #     data_dict['coords']=data_dict['coords'][trun_indice]
            #     data_dict['labels']=data_dict['labels'][trun_indice]
            ...
            c_score=jaccard_score(gt,preds,
                                labels=np.arange(config['num_labels']).tolist(),average='micro')
            train_score+=c_score
            train_ind_score.append(
                jaccard_score(gt,preds,labels=np.arange(config['num_labels']).tolist(),average=None))
            ...
            if curr_iter % config['val_freq']==0:
                ...
                with torch.no_grad():  # avoid out of memory problem
                    for data_dict in val_loader:
                        ...
                        # if(data_dict['feats'].size()[0]>config['train_limit_numpoint']):
                        #     indice=np.random.permutation(data_dict['feats'].size()[0])
                        #     trun_indice=indice[:config['train_limit_numpoint']]
                        #     data_dict['feats']=data_dict['feats'][trun_indice]
                        #     data_dict['coords']=data_dict['coords'][trun_indice]
